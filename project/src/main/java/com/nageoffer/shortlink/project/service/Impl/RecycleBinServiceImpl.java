...
public class RecycleBinServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements RecycleBinService {
    ...
    public void saveRecycleBin(RecycleBinSaveReqDTO requestParam) {
        ...
        ShortLinkDO shortLinkDO = ShortLinkDO.builder()
                .enableStatus(1) // 设置为未启用
                .build();
        baseMapper.update(shortLinkDO, updateWrapper);
        // 删除缓存GOTO_SHORT_LINK_KEY+ fullShortUrl，不然下次请求查到缓存会直接跳转
    ...
    public IPage<ShortLinkPageRespDTO> pageShortLink(ShortLinkRecycleBinPageReqDTO requestParam) {
        LambdaQueryWrapper<ShortLinkDO> queryWrapper = Wrappers.lambdaQuery(ShortLinkDO.class)
                ...
                .eq(ShortLinkDO::getEnableStatus, 1) // 1标识未启用，在回收站里
                .eq(ShortLinkDO::getDelFlag, 0)
    ...
    public void recoverRecycleBin(RecycleBinRecoverReqDTO requestParam) {
        LambdaUpdateWrapper<ShortLinkDO> updateWrapper = Wrappers.lambdaUpdate(ShortLinkDO.class)
                .eq(ShortLinkDO::getFullShortUrl, requestParam.getFullShortUrl())
                .eq(ShortLinkDO::getGid, requestParam.getGid())
                .eq(ShortLinkDO::getEnableStatus, 1) // 1是禁用了的，在回收站里面
                .eq(ShortLinkDO::getDelFlag, 0);
        ShortLinkDO shortLinkDO = ShortLinkDO.builder()
                .enableStatus(0) // 设置为启用，从回收站恢复
                .build();
        baseMapper.update(shortLinkDO, updateWrapper);
        // 删除redis缓存中的空key标记
    ...
    public void removeRecycleBin(RecycleBinRemoveReqDTO requestParam) {
        LambdaUpdateWrapper<ShortLinkDO> updateWrapper = Wrappers.lambdaUpdate(ShortLinkDO.class)
                .eq(ShortLinkDO::getFullShortUrl, requestParam.getFullShortUrl())
                .eq(ShortLinkDO::getGid, requestParam.getGid())
                .eq(ShortLinkDO::getEnableStatus, 1) // 1是禁用了的在回收站里面
                .eq(ShortLinkDO::getDelTime, 0L)
                .eq(ShortLinkDO::getDelFlag, 0);
        ShortLinkDO delShortLinkDO = ShortLinkDO.builder()
                .delTime(System.currentTimeMillis()) // 设置删除时间戳
                .build();
